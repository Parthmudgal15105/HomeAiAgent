...
The configured reasoning model chooses from named diagnostic tools. Python validation, configured target allowlists, a separate gateway, persisted approvals, and deterministic recovery checks enforce the boundary. Prompts guide reasoning but do not authorize operating-system access.
